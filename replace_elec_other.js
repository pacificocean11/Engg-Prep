const fs = require('fs');
const paths = [
    'd:\\\\Engg-Prep\\\\www\\\\questions.js',
    'd:\\\\Engg-Prep\\\\android\\\\app\\\\src\\\\main\\\\assets\\\\public\\\\questions.js'
];
...
for (const path of paths) {
    ...
    if(startIdx !== -1 && endIdx !== -1) {
        ...
        let localNewLines = newLines;
        if (lines[startIdx].endsWith('\r')) {
            localNewLines = newLines.map(l => l + '\r');
        }

        let finalLines = before.concat(localNewLines, after);
        fs.writeFileSync(path, finalLines.join('\n'), 'utf8');
        console.log('Successfully updated', path);
    } else {
        console.log('Error: Could not find startIdx or endIdx for', path);
    }
}
